...
void Connection::Kernel::receive() {
    int nl_length, atlen;
    char buf[8192];
    ...
    nl_length = this->receive_msg_headers(buf);
...
int Connection::Kernel::receive_msg_headers(char *buf) {
    char *ptr;
    int length;
    ...
    do {
        ...
        int received_bytes = recv(this->_socket, ptr, sizeof(buf) - length, 0);
